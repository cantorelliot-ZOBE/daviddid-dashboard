...
const pct = (cur: number | null, tgt: number | null) =>
  cur != null && tgt ? Math.min(100, Math.round((cur / tgt) * 100)) : 0
    ...
    <DashboardShell>
      <div className="text-muted text-[11px] font-mono tracking-widest uppercase">Today&apos;s Fuel</div>
        ...
        {!n || n.calories.current == null ? (
          <Empty className="mt-4" label="No nutrition logged yet — log meals in the app to see them here." />
        ) : (
          <>
            ...
            {[
              ...
              <div key={m.label} className="mb-4">
                ...
                <div className="h-1.5 bg-white/[0.08] rounded-full overflow-hidden mb-1.5">
                  <div className="h-full rounded-full" style={{ width: `${pct(m.current, m.target)}%`, background: m.color }} />
                </div>
            ...
            {n.meals.length === 0 ? (
              <Empty label="No meals logged yet today." />
            ) : (
            ...
            {n.davidNote && (
              <div className="bg-card2 border border-white/[0.06] border-l-2 border-l-lime rounded-card p-4 mt-4">
